...
var DDPClient = function(opts) {
  ...
  // 30sec is the default that was hardcoded into faye-websocket before I made
  // it configurable in the Tulip fork.
  self.closeTimeoutMs = typeof opts.closeTimeoutMs ==='number' ? opts.closeTimeoutMs : 30000;
...
DDPClient.prototype.connect = function(connected) {
  ...
  if (connected) {
    self._connectedListener = function() {
      self._clearReconnectTimeout();
    ...
    self.addListener("connected", self._connectedListener);

    self._failedListener = function(error) {
      self._isConnecting = false;
      self._connectionFailed = true;
      connected(error, self._isReconnecting);
    };

    self.addListener("failed", self._failedListener);
  }
...
DDPClient.prototype._makeWebSocketConnection = function(url) {
  var self = this;
  var options = { closeTimeoutMs: self.closeTimeoutMs };
...
DDPClient.prototype.close = function() {
  ...
  if (self._connectedListener) {
    self.removeListener("connected", self._connectedListener);
  }

  if (self._failedListener) {
    self.removeListener("failed", self._failedListener);
  }
};
